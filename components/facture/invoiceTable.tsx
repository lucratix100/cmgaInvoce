    ...
    return (
        <>
            <Filtre
                ...
                searchValue={search}
            />
            <div className="h-[calc(105vh-220px)] overflow-auto">
                            ...
                            {filteredInvoices.map((invoice) => (
                                <div key={invoice.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                                    <div className="flex justify-between items-start mb-2">
                                        <Link href={`/factures/${invoice.invoiceNumber}`} className="font-medium hover:text-primary transition-colors">
                                            {invoice.invoiceNumber}
                                        </Link>
                                        <StatusBadge status={invoice.status} />
                                    </div>
                                    <div className="space-y-1 text-sm text-gray-600">
                                        <p>N° Compte: {invoice.accountNumber}</p>
                                        <p>Date: {new Date(invoice.date).toLocaleDateString('fr-FR')}</p>
                                        <p>Client: {invoice.customer?.name || <span className="text-red-500">Non renseigné</span>}</p>
                                        <p>Téléphone: {invoice.customer?.phone || <span className="text-red-500">Non renseigné</span>}</p>
                                    </div>
                                    <div className="mt-3">
                                        <Button
                                            variant="ghost"
                                            size="sm"
                                            asChild
                                            className="w-full hover:bg-primary-100 hover:text-primary-700 transition-colors"
                                        >
                                            {user?.role === "ADMIN" ? (
                                                <Link href={`/dashboard/invoices/${invoice.invoiceNumber}`} className="flex items-center justify-center gap-2">
                                                    <Eye className="h-4 w-4" />
                                                    Détails
                                                </Link>
                                            ) : (
                                                <Link href={`/factures/${invoice.invoiceNumber}`} className="flex items-center justify-center gap-2">
                                                    <Eye className="h-4 w-4" />
                                                    Détails
                                                </Link>
                                            )}
